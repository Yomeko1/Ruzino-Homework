      ...
      _callEmission(HwShaderGenerator::ClosureContextType::EMISSION)
{
    ...
    //
    // Reflection context
    _callReflection.setSuffix(
        Type::BSDF, HwShaderGenerator::CLOSURE_CONTEXT_SUFFIX_REFLECTION);
    _callReflection.addArgument(
    ...
    _callIndirect.addArgument(
        Type::BSDF, ClosureContext::Argument(Type::VECTOR3, HW::DIR_V));
    // Emission context
    _callEmission.addArgument(
        Type::EDF, ClosureContext::Argument(Type::VECTOR3, HW::DIR_N));
    _callEmission.addArgument(
        Type::EDF, ClosureContext::Argument(Type::VECTOR3, HW::DIR_V));
}
...
void SurfaceNodeSlang::createVariables(
    const ShaderNode&,
    GenContext& context,
    Shader& shader) const
{
    // TODO:
    // The surface shader needs position, normal, view position and light
    // sources. We should solve this by adding some dependency mechanism so this
    // implementation can be set to depend on the HwPositionNode, HwNormalNode
    // HwViewDirectionNode and LightNodeSlang nodes instead? This is where the
    // MaterialX attribute "internalgeomprops" is needed.
    //
    ...
    addStageUniform(
        HW::PRIVATE_UNIFORMS,
        Type::MATRIX44,
        HW::T_WORLD_INVERSE_TRANSPOSE_MATRIX,
        vs);

    addStageConnector(
        HW::VERTEX_DATA, Type::VECTOR3, HW::T_POSITION_WORLD, vs, ps);
    addStageConnector(
        HW::VERTEX_DATA, Type::VECTOR3, HW::T_NORMAL_WORLD, vs, ps);

    addStageUniform(
        HW::PRIVATE_UNIFORMS, Type::VECTOR3, HW::T_VIEW_POSITION, ps);
...
void SurfaceNodeSlang::emitFunctionDefinition(
    ...
    ShaderStage& stage) const
{
    // const SlangShaderGenerator& shadergen =
    //     static_cast<const
    //     SlangShaderGenerator&>(context.getShaderGenerator());
    ...
    //    for (auto* input : parent->getInputs()) {
    //        shadergen.emitInput(input, context, stage);
    //        shadergen.emitString(", ", stage);
    //    }

    //    shadergen.emitLine(" out BSDF result)", stage, false);
    ...
    ShaderStage& stage) const
{
    ...
    DEFINE_SHADER_STAGE(stage, Stage::VERTEX)
    {
        ...
        if (!position->isEmitted()) {
            position->setEmitted();
            shadergen.emitLine(
                prefix + position->getVariable() + " = hPositionWorld.xyz",
                stage);
        }
    ...
    DEFINE_SHADER_STAGE(stage, Stage::PIXEL)
    {
        ...
        if (bsdf) {
            ...
            if (context.getOptions().hwShadowMap) {
                ...
                shadergen.emitLine(
                    "float occlusion = "
                    "mx_variance_shadow_occlusion(shadowMoments, "
                    "shadowCoord.z)",
                    stage);
            }
            else {
...
void SurfaceNodeSlang::emitLightLoop(
    const ShaderNode& node,
    GenContext& context,
    ShaderStage& stage,
    const string& outColor) const
{
    ...
    if (context.getOptions().hwMaxActiveLightSources > 0) {
        const SlangShaderGenerator& shadergen =
            static_cast<const SlangShaderGenerator&>(
