...
void BitangentNodeSlang::emitFunctionCall(
        ...
        if (space == WORLD_SPACE) {
            ...
            if (!bitangent->isEmitted()) {
                ...
                if (options.hwImplicitBitangents) {
                    ...
                    if (!normal->isEmitted()) {
                        normal->setEmitted();
                        shadergen.emitLine(
                            prefix + normal->getVariable() +
                                " = normalize(mul(" +
                                HW::T_WORLD_INVERSE_TRANSPOSE_MATRIX +
                                ", float4(" + HW::T_IN_NORMAL + ", 0.0)).xyz)",
                            stage);
                    }
                    ...
                    if (!tangent->isEmitted()) {
                        tangent->setEmitted();
                        shadergen.emitLine(
                            prefix + tangent->getVariable() +
                                " = normalize(mul(" + HW::T_WORLD_MATRIX +
                                ", float4(" + HW::T_IN_TANGENT + ", 0.0)).xyz)",
                            stage);
                    }
                    shadergen.emitLine(
                        prefix + bitangent->getVariable() + " = cross(" +
                            prefix + normal->getVariable() + ", " + prefix +
                            tangent->getVariable() + ")",
                        stage);
                }
                else {
                    shadergen.emitLine(
                        prefix + bitangent->getVariable() + " = normalize((" +
                            HW::T_WORLD_MATRIX + " * float4(" +
                            HW::T_IN_BITANGENT + ", 0.0)).xyz)",
                        stage);
                }
            }
        }
            ...
            if (!bitangent->isEmitted()) {
                ...
                if (options.hwImplicitBitangents) {
                    shadergen.emitLine(
                        prefix + bitangent->getVariable() + " = cross(" +
                            HW::T_IN_NORMAL + ", " + HW::T_IN_TANGENT + ")",
                        stage);
                }
                else {
                    shadergen.emitLine(
                        prefix + bitangent->getVariable() + " = " +
                            HW::T_IN_BITANGENT,
                        stage);
                }
            }
    ...
    DEFINE_SHADER_STAGE(stage, Stage::PIXEL)
    {
        ...
        if (space == WORLD_SPACE) {
            ...
            shadergen.emitString(
                " = normalize(" + prefix + bitangent->getVariable() + ")",
                stage);
        }
        else {
