...
class ShaderGeneratorTester {
   ...
   protected:
    ...
    std::set<std::string> successed_files;
...
inline bool ShaderGeneratorTester::compileSource(
    ...
    const std::string& optionsFilePath)
{
    ...
    for (const auto& doc : _documents) {
        if (doc->getSourceUri().empty()) {
            _logFile << "Empty document path." << std::endl;
            continue;
        }
                    ...
                    if (!generatedCode) {
                        ...
                        if (compileSource(sourceCodePaths)) {
                            ...
                            if (documentIndex % 10 == 0) {
                                std::ofstream successed_file_log_name(
                                    "successed_files.txt");
                                for (const auto& file : successed_files) {
                                    successed_file_log_name << file
                                                            << std::endl;
                                }
                                successed_file_log_name.close();
                            }
                        }
                    }
                }
...
class SlangShaderGeneratorTester : public GenShaderUtil::ShaderGeneratorTester {
   public:
    ...
    SlangShaderGeneratorTester(
        ...
        const mx::FilePath& logFilePath,
        bool writeShadersToDisk)
        : GenShaderUtil::ShaderGeneratorTester(
              shaderGenerator,
              testRootPaths,
              searchPath,
              logFilePath,
    ...
    void setupDependentLibraries() override
    {
        ...
        mx::FilePath lightDir = mx::getDefaultDataSearchPath().find(
            "resources/Materials/TestSuite/lights");
        loadLibrary(
            lightDir / mx::FilePath("light_compound_test.mtlx"), _dependLib);
        loadLibrary(
            lightDir / mx::FilePath("light_rig_test_1.mtlx"), _dependLib);
    }

   protected:
    void getImplementationWhiteList(mx::StringSet& whiteList) override
    {
        whiteList = { "ambientocclusion",
                      "arrayappend",
                      "screen",
                      "curveadjust",
                      "displacementshader",
