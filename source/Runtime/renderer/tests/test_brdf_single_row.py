...
def analyze_middle_row(eval_array, sample_array, importance_array, valid_mask, material_name):
    ...
    print(f"{'='*80}")
    print(f"Row {best_result['row']}: L2={best_result['l2_distance']:.6e}, Corr={best_result['correlation']:.3f}, CV={best_result['importance_cv']:.3f}")
...
def main():
    ...
    try:
        ...
        eval_exr = np.flipud(eval_array[:, :, :3].astype(np.float32))
        sample_exr = np.flipud(sample_array[:, :, :3].astype(np.float32))
        importance_exr = np.flipud(importance_array[:, :, :3].astype(np.float32))
        
        # Save to output directory
        imageio.imwrite(output_dir / f"{material_name}_eval.exr", eval_exr, format="EXR")
        imageio.imwrite(output_dir / f"{material_name}_sample.exr", sample_exr, format="EXR")
        imageio.imwrite(output_dir / f"{material_name}_importance.exr", importance_exr, format="EXR")
    ...
    # Analyze middle row
    results = analyze_middle_row(eval_array, sample_array, importance_array, valid_mask, material_name)
